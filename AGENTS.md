...
2026-09-11 用户驱动改造：七工位键保留，主线调整为剧本库 → 素材定义 → 分段编写 → 生成选片 → 初剪交付；`SegmentWriter` 取代默认 AI 导演页，角色和场景道具由片段显式选择，毫秒动作时间线写进统一提示词与参考槽。旧最终执行稿须审阅采用后才替换。Codex 已有虚拟 chat/image 双槽，优化、识图和创作助手共用 `chatStream`。Eagle 离线静默保留队列，恢复后集中续传。独立平面设计工作台入口已移除。剪辑交付支持 FCP7 XML 与 pyJianYingDraft 草稿，兼容边界和验收见 `docs/创作链路与导演台写作改造-2026-09-10.md`。
