...
export function CodexBridgeCard(){
 ...
 <div className="codex-bridge-actions"><button className="btn sm" disabled={status?.account.type!=="chatgpt"} onClick={()=>{useSettings.getState().setDefault("chat","codex-membership::codex-chat");useSettings.getState().setDefault("image","codex-membership::codex-image");}}>用于对话与生图</button><button className="btn sm" disabled={busy||!isTauri} onClick={()=>void refresh()}>{busy?"连接中…":"检查连接与额度"}</button><button className="btn sm" disabled={busy||!isTauri} onClick={()=>void choose()}>选择 Codex 程序</button><button className="btn sm" onClick={()=>void openExternal("https://learn.chatgpt.com/docs/auth")}>登录说明</button><button className="btn sm" disabled={status?.account.type!=="chatgpt"||defaults.image==="codex-membership::codex-image"} onClick={()=>useSettings.getState().setDefault("image","codex-membership::codex-image")}>{defaults.image==="codex-membership::codex-image"?"已用于默认生图":"设为默认生图"}</button></div>
