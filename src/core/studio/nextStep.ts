...
export function nextStudioStep(project: DirectorProject) {
  ...
  if (!segments.length) return { station: "scripts" as const, label: "导入剧本", hint: "已有剧本直接导入；只有想法可先到 AI 导演共创。", stage: 0 };
  ...
  if (empty) return { station: "h3" as const, segmentId: empty.id, label: "补全分镜内容", hint: "先补这一段的描述或执行提示词，再选择配方。", stage: 1 };
  ...
  if (unfinished) {
    ...
    return { station: "h3" as const, segmentId: unfinished.id, label: done ? "挑选采用版本" : "检查并生成片段", hint: done ? "这一段已有结果，选中满意版本并采用。" : "确认提示词、参考素材和配方；有需要时再用角色库、制图或 3D。", stage: 2 };
  }
