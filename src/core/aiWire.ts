...
- videoGen 生成视频（data.prompt 可留空=自动用上游文本）
- outpaint 扩图（上游需图片）
...
连线规则：
- 端口分 text（文本）与 image（图片）两类，只能同类相连。
- 各节点输出类型：prompt/caption/llmText/combine/stylePreset→text；image/imageGen/outpaint/matting/enhance/resize/relight/multiAngle/charCard→image；videoGen/note 无输出。
- 各节点可接收：imageGen/videoGen/outpaint/relight/multiAngle/charCard 可接 text+image；caption/matting/enhance/resize 只接 image；llmText/combine 只接 text；image/prompt/stylePreset/note 无输入。
...
export async function buildWirePlan(intent: string): Promise<WirePlan> {
  ...
  for (const e of raw.edges ?? []) {
    ...
    if (!pt || pt === "video") continue;
    const ins = NODE_INPUTS[to.kind];
    if (!(pt === "text" ? ins.text : ins.image)) continue;
...
export function applyWirePlan(plan: WirePlan, at: { x: number; y: number }) {
  ...
  const edges: Edge[] = plan.edges.map((e) => ({
    ...
    sourceHandle: "out",
    targetHandle: e.port === "image" ? "in-image" : "in-text",
