...
- videoConcat 视频拼接：多路上游视频按顺序合成一条成片
- outpaint 扩图（上游需图片）
...
连线规则：
- 端口分 text（文本）、image（图片）、video（视频）三类，只能同类相连。
- 各节点输出类型：prompt/caption/llmText/combine/stylePreset/storyboard→text；image/imageGen/outpaint/matting/enhance/resize/relight/multiAngle/charCard/frame→image；video/videoGen/videoTrim/videoConcat→video；note 无输出。
- 各节点可接收：imageGen/relight/multiAngle/charCard/storyboard/outpaint 可接 text+image；videoGen 可接 text+image+video；caption/matting/enhance/resize 只接 image；llmText/combine 只接 text；frame/videoTrim/videoConcat 只接 video；image/video/prompt/stylePreset/note 无输入。
- 生成图像的提示词留空时会自动使用上游文本；接了上游图片会自动转图生图。
- 典型视频链：storyboard→videoGen（多个）→videoConcat 成片；videoGen→frame（末帧）→下一个 videoGen 首帧实现续接。
- 不允许成环。
...
export async function buildWirePlan(intent: string): Promise<WirePlan> {
  ...
  for (const e of raw.edges ?? []) {
    ...
    if (!(pt === "text" ? ins.text : pt === "video" ? ins.video : ins.image)) continue;
...
export function applyWirePlan(plan: WirePlan, at: { x: number; y: number }) {
  ...
  const edges: Edge[] = plan.edges.map((e) => ({
    ...
    sourceHandle: "out",
    targetHandle: e.port === "image" ? "in-image" : e.port === "video" ? "in-video" : "in-text",
